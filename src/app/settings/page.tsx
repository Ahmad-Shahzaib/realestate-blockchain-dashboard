...
export default function SettingsPage() {
  ...
  return (
    <div className="p-6 mx-auto bg-background border border-themebgColor text-black">
      <h1 className="text-2xl font-semibold mb-4 text-black">Settings</h1>
      <div className="flex lg:flex-row flex-col gap-8">
        {/* Tabs column */}
        <div className="border-l border-themebgColor max-w-xs min-w-[200px]">
          <nav className="flex lg:flex-col flex-row overflow-x-auto gap-2 text-sm font-medium text-black">
            {tabs.map((tab, index) => (
              <button
                key={index}
                onClick={() => setActiveTab(index)}
                className={`text-left px-4 py-3 rounded-l-md transition w-full ${activeTab === index
                  ? "bg-background font-semibold border-l-4 border-themebgColor text-black"
                  : "hover:text-black"
                  }`}
              >
                {tab}
